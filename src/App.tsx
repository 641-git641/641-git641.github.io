...
const blogPosts: BlogPost[] = [
  ...
  },
  {
     ...
     sections: [
       {
         level: 1,
         heading: 'AI 化设施不是模型单点',
         paragraphs: [
           'AI 化设施不只是 GPU 集群和模型服务，还包括数据采集、数据治理、标注平台、特征与向量存储、训练评测、推理网关以及线上监控。',
           '模型能力的上限通常由数据质量决定。数据来源不清、标签错误、权限失控、知识过期，都会最终表现为模型幻觉、错误决策或安全事故。因此，AI 化设施必须建立数据评审闭环，而不是只在模型上线前做一次检查。',
         ],
       },
       {
         level: 1,
         heading: '为什么必须建立数据评审闭环',
         paragraphs: [
           '一次性审批只能确认某个时间点的状态，不能保证数据长期有效。数据源会变化，业务规则会变化，用户反馈会变化，知识库内容也会过期。',
           '完整的数据评审闭环应覆盖四个阶段：数据进入设施前的准入评审；数据清洗、标注和加工过程中的质量评审；数据集进入训练、检索和评测流程前的发布评审；模型上线后的问题回流和数据修复。',
           '数据评审不是把所有数据都交给人工逐条检查，而是通过自动规则、风险分级、抽样检查和人工复核共同完成。低风险数据自动通过，高风险数据必须经过人工审批。',
         ],
       },
       {
         level: 1,
         heading: '数据进入设施前：准入评审',
       },
       {
         level: 2,
         heading: '来源与授权',
         paragraphs: [
           '首先确认数据从哪里来、由谁提供、允许做什么。评审内容包括数据授权、使用范围、保存期限、第三方许可和跨区域传输限制。',
           '对于用户内容、业务日志和外部数据，必须明确是否允许用于训练、检索、分析或仅用于故障排查。无法证明来源和授权关系的数据，应进入隔离区，而不是直接进入生产数据池。',
         ],
       },
       {
         level: 2,
         heading: '质量与敏感性',
         paragraphs: [
           '数据接入前需要检查格式、字段、完整性、重复率、时间新鲜度和异常比例。同时识别个人信息、密钥、内部配置、商业机密等敏感内容。',
           '敏感等级决定数据的存储位置、访问权限、脱敏方式和评审人。数据越敏感，评审链路越长，留痕要求越高。',
         ],
       },
       {
         level: 1,
       ...
       },
       {
         ...
         heading: '清洗与标注',
         paragraphs: [
           '清洗规则、标注规范和抽样比例都应该版本化。标注任务需要记录操作人、复核人、争议样本和最终结论。',
           '对于高风险数据，不能只依赖单次标注结果。应通过双人标注、抽样复核或专家复核降低标签错误率，并把争议样本沉淀为新的评审规则。',
         ],
       },
       {
         level: 2,
         heading: '防泄漏与防投毒',
         paragraphs: [
           '训练集、验证集和测试集必须隔离，避免测试数据泄漏到训练过程。用户输入、网页内容和第三方文档中可能包含提示注入、恶意指令或污染样本，也需要在进入数据集前进行检测。',
           '异常数据不应被直接删除。更合理的做法是先进入隔离区，保留来源和证据，完成调查后再决定放行、修复或拒绝。',
         ],
       },
       {
         level: 1,
         heading: '数据集版本与可追溯性',
         paragraphs: [
           '数据集应该像软件制品一样管理。每次发布都需要拥有唯一版本号、内容摘要、来源记录、处理代码、规则版本、评审人和发布时间。',
           '已经发布的数据集不能直接覆盖。任何修改都应该生成新版本，并保留新旧版本之间的差异。模型、知识库和评测结果必须能够反向追溯到具体的数据集版本。',
           '这样出现问题时，团队才能准确回答三个问题：哪些数据影响了结果、谁批准了这批数据、如何回滚到上一个稳定版本。',
         ],
       },
       {
         level: 1,
         heading: '训练、检索与评测分开治理',
       },
       {
         level: 2,
         heading: '训练数据',
         paragraphs: [
           '训练数据重点关注代表性、标签质量、数据泄漏和敏感信息。生产用户数据不能未经脱敏就直接用于训练，也不能因为数据量不足而跳过授权和评审。',
         ],
       },
       {
         level: 2,
         heading: '检索数据',
         paragraphs: [
           'RAG 或知识库系统要重点检查来源权限、内容时效、访问控制和删除同步。用户没有权限看到的文档，即使已经进入向量库，也不能被检索结果返回。',
           '文档更新或撤回后，向量索引必须同步更新。否则原文已经删除，模型仍然可能通过旧向量生成过期答案。',
         ],
       },
       {
         level: 2,
         heading: '评测数据',
         paragraphs: [
           '评测集不能完全来自训练集。它应该覆盖正常场景、边界场景、异常输入和高风险业务，并且保持独立版本。',
           '评测结果不能只看平均准确率，还应该关注高风险错误、拒答能力、敏感信息泄露、幻觉率和不同用户群体之间的表现差异。',
         ],
       },
       {
         level: 1,
         heading: '上线后的反馈回流',
         paragraphs: [
           '模型上线后，系统需要持续收集低质量回答、用户纠错、人工拦截、权限违规、数据过期和异常调用等反馈。',
           '反馈数据进入问题队列后，需要经过分类、定级和分派。问题可能对应数据错误、提示词问题、检索权限问题、模型能力不足或监控规则缺失，不能全部归因于模型本身。',
           '每个高风险问题都应该有责任人、处理期限、修复方案和验证结果。修复完成后，问题不能只标记为关闭，还应将案例加入新的评测集或检查规则，防止同类问题再次出现。',
         ],
       },
       {
         level: 1,
         heading: '把评审做成状态机',
         paragraphs: [
           '一套可执行的状态流转可以是：草稿 -> 自动校验 -> 人工评审 -> 通过发布 -> 线上监测 -> 问题回流 -> 修复验证 -> 新版本归档。',
           '评审流程需要根据风险分级。低风险数据可以自动校验后发布；涉及个人信息、金融决策、医疗建议、核心业务规则的数据，必须经过人工审批和完整审计。',
           '评审的目标不是拖慢数据流动，而是让风险在进入生产之前被发现，让问题在进入生产之后能够被追踪和修复。',
         ],
       },
       {
         level: 1,
         heading: '用指标判断闭环是否有效',
         paragraphs: [
           '建议持续关注数据评审覆盖率、自动校验通过率、人工拒绝率、线上问题逃逸率、高风险问题关闭时长、同类问题重复发生率、数据新鲜度、数据血缘完整率以及数据集和模型回滚成功率。',
           '指标的作用不是增加报表，而是发现流程瓶颈。如果评审覆盖率很高，但线上问题逃逸率仍然上升，说明评审规则没有覆盖真实风险。',
         ],
       },
       {
         level: 1,
         heading: '常见误区',
         paragraphs: [
           '第一个误区是只评审训练数据，不评审检索数据和线上反馈。实际上，知识库和实时数据同样会直接影响模型输出。',
           '第二个误区是只看模型准确率，不看数据来源、权限、敏感信息和高风险错误。模型平均指标很好，不代表系统在关键场景下安全。',
           '第三个误区是没有明确的数据责任人，或者直接覆盖旧数据集。没有责任人的数据无法及时修复，直接覆盖则会破坏血缘关系和回滚能力。',
         ],
       },
       {
         level: 1,
         heading: '结语',
         paragraphs: [
           'AI 化设施的核心竞争力不只是更大的模型和更多的算力，还包括稳定、透明、可追溯的数据治理能力。数据评审闭环的最终目标，是让每一份数据都能说明来源，让每一次发布都有依据，让每一个线上问题都能回到数据、规则或模型，并最终转化为下一轮改进。',
         ],
       },
     ],
   },
